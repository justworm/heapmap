...
(function(w){
	...
	var heatmapFactory = (function(){
		...
		// the store has to know about the heatmap in order to trigger heatmap updates 
		// when datapoints get added
		...
		store.prototype = {
			...
			// datapoints are usually defined by x and y but could also contain a third parameter 
			// which represents the occurrence
		...
		legend.prototype = {
			init: function(){
				...
				gradient.style.cssText = ["\
					...
					background-image:url(",me.createGradientImage(),");\
				"].join("");
			...
			processGradientObject: function(){
				...
				//遍历gradientConfig对象里面的每一个值；以数组形式重新存入gradientArr中
			...
			update: function(max){
			    ...
			    for(var i = 0; i < gradient.length; i++){
			        ...
			        labelsHtml += '<li style="\
				        position:absolute;\
				        left:'+(((((1/(gradient.length-1)*i*256) || 0)) >> 0)-offset+.5)+'px\
			        ">'+labelText+'</li>';
			    }       
		...
		heatmap.prototype = {
			configure: function(config){
				...
				me.set("element", (
					config.element instanceof Object)
					?config.element
			...
			init: function(){
				...
				me.resize();
				canvas.style.cssText = acanvas.style.cssText = "\
					position:absolute;\
					top:0;left:0;\
					z-index:10000000;\
				";
			...
			colorize: function(x, y){
				...
				// after the manipulation process we have to set the manipulated data 
				// to the ImageData object
				// 重新设置回data值
		...
		return {
			create: function(config){
				return new heatmap(config);  //创建了闭包 : heatmap; legend; store
			}, 
			util: {
